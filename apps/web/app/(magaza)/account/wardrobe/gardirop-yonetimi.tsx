    ...
    <li className="flex flex-col gap-2">
      <div className="relative aspect-urun w-full overflow-hidden rounded-md bg-urun-zemin">
        {parca.imageUrl ? (
           ...
           *
           * ⚠️ `gorsel-yedek` BURADA DA ZORUNLU ve sebebi `<UrunGorseli>`den
           *    FARKLI: adres imzalı ve imza 900 sn sonra ölüyor. Sekme açık
           *    kalırsa görsel bir noktadan sonra 403 dönmeye başlar ve burada
           *    hiçbir `onError` yolu YOK — kırık ikon kalıcı olurdu.
           *    (`deneme-gorseli.tsx` aynı imza ölümünü ele alıyordu, gardırop
           *    almıyordu.) Örtü saf CSS olduğu için JS'e hiç dokunmadan
           *    kapanıyor; gerekçenin tamamı `globals.css` → `.gorsel-yedek`.
           */
          // eslint-disable-next-line @next/next/no-img-element
          <img
            src={parca.imageUrl}
            alt={parca.label ?? GARDIROP_KATEGORISI[parca.category]}
            data-yedek-ad={parca.label ?? GARDIROP_KATEGORISI[parca.category]}
            className="gorsel-yedek size-full object-cover"
            loading="lazy"
          />
        ) : (
          /* Adres hiç yoksa kutu BOŞ kalıyordu — aynı yedek burada da çizilir. */
          <span
            role="img"
            aria-label={`${parca.label ?? GARDIROP_KATEGORISI[parca.category]} — görsel yüklenemedi`}
            data-yedek-ad={parca.label ?? GARDIROP_KATEGORISI[parca.category]}
