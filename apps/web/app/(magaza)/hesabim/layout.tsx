 ...
 *
 * ⚠️ `proxy.ts` matcher'ı ARTIK BU YOLU KAPSIYOR (`/hesabim/:path*`), ama
 ...
 *    guard'ları. Matcher'ın tek kazancı ölçülebilir: çerezsiz `/hesabim`
 *    isteği artık gövdeye hiç girmeden HTTP 307 ile `/giris?next=/hesabim`e
 ...
 *    burada rol değil OTURUM aranıyor ve doğru cevap 404 değil, `/giris`e
...
export default async function HesapLayout({ children }: { children: ReactNode }) {
  ...
  if (!kullanici) redirect('/giris?next=/hesabim');
