 ...
 *
 * Kalıp `(magaza)/urunler/page.tsx`ten alındı: Sunucu Bileşeni, veri DOĞRUDAN
 ...
 *
 *     /koleksiyon/canta 200   /koleksiyon/yok-boyle 200   /koleksiyon/xyz 200
 *
 ...
 *    yönlendirici düzeyinde bir kapı KURMUYOR. Yani bayrak `hukuki/[belge]`
...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  const yol = `/koleksiyon/${kayit.slug}`;
...
export default async function KoleksiyonPage({ params }: { params: Params }) {
  ...
  const tumunuGorAdresi = `/urunler?q=${encodeURIComponent(kayit.aramaTerimi)}${
    ...
    <section>
      ...
      {items.length === 0 ? (
        ...
        <div className="rounded-md border border-kenar bg-yuzey p-6 text-sm">
          ...
          <Link href="/urunler" className="mt-2 inline-block text-vurgu hover:underline">
