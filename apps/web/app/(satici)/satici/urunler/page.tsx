 ...
 *
 * Ekranın tüm durumu URL'dedir (durum sekmesi, arama, imleç); `/urunler`
 ...
 *    kimliğin arkasında ve `hesapFetch` oturumsuz kullanıcıyı `/giris`e atıyor.
...
function baglanti(params: AramaParametreleri): string {
  ...
  return qs ? `/satici/urunler?${qs}` : '/satici/urunler';
}
...
export default async function SaticiUrunlerPage({
  ...
  return (
    <section>
      ...
      */}
      <form action="/satici/urunler" className="mt-4 flex gap-2">
  ...
  if (aranan) {
    return (
      <BosSonuc
        ...
        eylem={
          <Button asChild variant="ikincil" size="sm">
            <Link href={durum ? `/satici/urunler?durum=${durum}` : '/satici/urunler'}>
              Aramayı temizle
            </Link>
          </Button>
        }
  ...
  if (durum) {
    return (
      <BosSonuc
        ...
        eylem={
          <Button asChild variant="ikincil" size="sm">
            <Link href="/satici/urunler">Tüm ürünleri göster</Link>
          </Button>
        }
      />
    );
    ...
    <BosSonuc
      ...
      eylem={
        <div className="flex flex-wrap gap-2">
          <Button asChild size="sm">
            <Link href="/satici/urunler/yeni">Ürün ekle</Link>
          </Button>
          <Button asChild variant="ikincil" size="sm">
            <Link href="/satici/urunler/toplu-yukleme">CSV ile toplu yükle</Link>
          </Button>
        </div>
      }
    />
