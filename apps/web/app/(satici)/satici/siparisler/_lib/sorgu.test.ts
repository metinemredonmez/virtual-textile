...
describe('suzgecYolu', () => {
  it('süzgeç değişince imleci TAŞIMAZ', () => {
    ...
    expect(suzgecYolu(sorgu, { durum: 'SHIPPED' })).toBe('/satici/siparisler?durum=SHIPPED');
  });

  it('süzgeçsiz yol sorgu dizesi taşımaz', () => {
    expect(suzgecYolu(sorguyuOku({}), {})).toBe('/satici/siparisler');
  });

  it('sipariş numarası aramasını korur', () => {
    const sorgu = sorguyuOku({ siparisNo: 'VT-1' });
    expect(suzgecYolu(sorgu, { durum: 'SHIPPED' })).toBe(
      '/satici/siparisler?durum=SHIPPED&siparisNo=VT-1',
    );
  });
});
...
describe('imlecYolu', () => {
  it('süzgeçleri koruyarak imleç ekler', () => {
    ...
    expect(imlecYolu(sorgu, 'im/leç')).toBe(
      '/satici/siparisler?durum=PREPARING&imlec=im%2Fle%C3%A7',
    );
  });

  it('süzgeç yokken ayracı ? olarak kurar', () => {
    expect(imlecYolu(sorguyuOku({}), 'abc')).toBe('/satici/siparisler?imlec=abc');
  });
});
