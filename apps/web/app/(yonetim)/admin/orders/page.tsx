...
export default async function SiparislerPage({
  ...
  try {
    const sonuc = await hesapFetch<unknown, '/admin/orders'>('/admin/orders', '/admin/orders', {
      query: {
        limit: SAYFA_BOYU,
            ...
            {siparisler.map((siparis) => {
              ...
              return (
                <TR key={siparis.id}>
                  <TD>
                    <Link
                      href={`/admin/orders/${encodeURIComponent(siparis.orderNumber)}`}
