...
export default async function KomisyonPage({
  ...
  try {
    ...
    const [kuralSonucu, kategoriSonucu, saticiSonucu] = await Promise.all([
      ...
      hesapFetch<unknown, '/admin/categories'>('/admin/categories', '/admin/commission'),
      hesapFetch<unknown, '/admin/sellers'>('/admin/sellers', '/admin/commission', {
          ...
          <TBody>
            {gosterilen.map((kural) => (
              <TR key={kural.id}>
                ...
                <TD>
                  <Link
                    href={`/admin/commission/${kural.id}`}
